  ...
  struct
    ...
    let fold_left f acc sa =
      let rec aux acc index i =
        if i < sa.index
        then aux (f acc sa.symarray.data.(index)) (succ index) (succ i)
        else if i < sa.symarray.length
        then aux (f acc sa.symarray.data.(index)) (succ index + i) (succ i)
        else acc
      in
    ...
    let fold_lefti f acc sa =
      let rec aux acc index i =
        if i < sa.index
        then aux (f acc i sa.symarray.data.(index)) (succ index) (succ i)
        else if i < sa.symarray.length
        then aux (f acc i sa.symarray.data.(index)) (succ index + i) (succ i)
        else acc
      in
      aux acc (sa.index * (sa.index + 1) / 2) 0
  end
...
let make l e =
  ...
  with Invalid_argument _ -> invalid_arg "SymmetricArray.make"
